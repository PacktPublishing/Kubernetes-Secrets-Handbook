...
## Kube-Bench
Kube-bench is a benchmarking tool that allows to have a rapid overview of how your Kubernetes cluster complies to a specific security framework. 

To install kube-bench, use the job file called ```job-fullcis.yaml``` in this directory and execute the following command:

```bash
kubectl apply -f job-fullcis.yaml
```
resulting with
```
...
kube-bench-gsfg6   0/1     Completed   0          32s
```

Grab the pod node and get the logs with:
```
kubectl logs kube-bench-gsfg6
```
resulting with the following output being the benchmark:
```
...
To generate a local report, run the following command:

```
kkubectl logs kube-bench-gsfg6 > kube-bench-report-$(date +%Y-%m-%d_%H:%M).txt
```
resulting in the creation of a file ```kube-bench-report-2023-10-15_19:02.txt``` saved locally to be reviewed later. 
